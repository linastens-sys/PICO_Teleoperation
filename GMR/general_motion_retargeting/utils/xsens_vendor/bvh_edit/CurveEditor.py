...
class OffsetManager:
  ...
  def format_for_save(self, offsets, joint_names):
    ...
    save_data = {}
    for j, joint in enumerate(joint_names):
      save_data[joint] = {channel_names[c]: offsets.get((j, c), 0.0) for c in range(3)}
    return save_data
...
class CurveEditorWindow(QMainWindow):
  ...
  def on_apply_preview(self):
    ...
    _quats, _positions, _offsets, _parents = self.parser._MOTION_data_post_processing(
  ...
  def get_channel_data(self):
    ...
    current_offset = self.offsets[(self.selected_joint_idx, self.selected_channel_idx)]
