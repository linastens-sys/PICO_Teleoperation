...
class BVHParser:
  ...
  def _HIERARCHY_paser(self, line_idx=-1):
    try:
      ...
      if self.line.startswith("ROOT"):
        ...
        name = self.line.split()[1]
        if not self.stack:
          raise ValueError(f"JOINT {name} found before ROOT or outside hierarchy")
        node = Node(name)
  ...
  def generate_mujoco_xml(self, frame_0=[]):
    def generate_xml(node, indent=2):
      ...
      if node.name == "Hips":
        xml += f'{spaces}  <geom type="sphere" size="{str(self.r * 1.5)}" rgba="0.5 0.0 1.0 0.5"/>\n'
      elif node.name.endswith("_end_site"):
        xml += f'{spaces}  <geom type="sphere" size="{str(self.r * 1.5)}" rgba="1.0 0.5 0.0 0.5"/>\n'
      else:
        xml += f'{spaces}  <geom type="sphere" size="{str(self.r * 1.5)}" rgba="0.8 0.8 0.8 0.5"/>\n'
      for child in node.children:
        v = np.array(child.offset)
        l = np.linalg.norm(v)
        pos_str = " ".join(f"{x / 2:.6f}" for x in child.offset)
        q_xyzw = (
          R.align_vectors([v / l], [[0, 0, 1]])[0].as_quat(scalar_first=True).tolist()
        )
        q_str = " ".join(f"{x / 2}" for x in q_xyzw)
        xml += f'{spaces}  <geom type="capsule" size="{str(self.r)} {str(np.clip(l * 0.5 - self.r * 2, min=0) + 1e-5)}" pos="{pos_str}"  quat="{q_str}"  rgba="1.0 0.5 1.0 0.5"/>\n'
