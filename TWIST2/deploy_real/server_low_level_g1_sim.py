...
def load_onnx_policy(policy_path: str, device: str) -> OnnxPolicyWrapper:
  if ort is None:
    raise ImportError(
      "onnxruntime is required for ONNX policy inference but is not installed."
    )
  providers = []
  ...
  print(
    f"ONNX policy loaded from {policy_path} using providers: {session.get_providers()}"
  )
...
class RealTimePolicyController:
  ...
  def reset(self, init_pos):
    ...
    self.data.qpos[: len(init_pos)] = init_pos  # tolerate extra (bag) DOFs
    self.data.qvel[:] = 0
    mujoco.mj_forward(self.model, self.data)
  ...
  def run(self):
    ...
    self.redis_pipeline.set(
      "state_body_unitree_g1_with_hands", json.dumps(initial_obs.tolist())
    )
    self.redis_pipeline.set(
      "state_hand_left_unitree_g1_with_hands", json.dumps(np.zeros(7).tolist())
    )
    self.redis_pipeline.set(
      "state_hand_right_unitree_g1_with_hands", json.dumps(np.zeros(7).tolist())
    )
    ...
    try:
      for i in pbar:
        ...
        if i % self.sim_decimation == 0:
          ...
          state_body = np.concatenate([ang_vel, rpy[:2], dof_pos])  # 3+2+29 = 34 dims

          # Send proprio to redis

          self.redis_pipeline.set(
            "state_body_unitree_g1_with_hands", json.dumps(state_body.tolist())
          )
          self.redis_pipeline.set(
            "state_hand_left_unitree_g1_with_hands", json.dumps(np.zeros(7).tolist())
          )
          ...
          self.redis_pipeline.set(
            "state_neck_unitree_g1_with_hands", json.dumps(np.zeros(2).tolist())
          )
          self.redis_pipeline.set(
          ...
          if last_policy_time is not None:
            ...
            if policy_step_count % policy_fps_print_interval == 0:
              ...
              print(
                f"Policy Execution FPS (last {policy_fps_print_interval} steps): {avg_execution_fps:.2f} Hz (avg interval: {avg_interval * 1000:.2f}ms)"
              )
            ...
            if measure_fps:
              ...
              if fps_iteration_count == fps_measurement_target:
                ...
                print(
                  f"Expected FPS (from decimation): {1.0 / (self.sim_decimation * self.sim_dt):.2f}"
                )
                print(
                  f"=================================================================================\n"
                )
        ...
        # --- Bag collision monitor (edge-triggered; robot part + force) ---
        bag_bid = getattr(self, "_bag_bid", -2)
        if bag_bid == -2:
          bag_bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "bag")
          self._bag_bid = bag_bid
        if bag_bid >= 0:
          hit, max_f, part = False, 0.0, "?"
          ...
          for i in range(self.data.ncon):
            ...
            if bag_bid in (b1, b2):
              hit = True
              other = c.geom2 if b1 == bag_bid else c.geom1
              mujoco.mj_contactForce(self.model, self.data, i, ft)
              f = float(np.linalg.norm(ft[:3]))
              if f > max_f:
                max_f = f
                part = (mujoco.mj_id2name(
                  self.model, mujoco.mjtObj.mjOBJ_BODY,
                  int(self.model.geom_bodyid[other])) or "?")
          was = getattr(self, "_bag_hit", False)
          if hit and not was:
            print(f"[BAG] collision START vs {part}  |F|={max_f:.1f} N", flush=True)
          elif not hit and was:
            print("[BAG] collision end", flush=True)
          self._bag_hit = hit
        # --- end bag collision monitor ---
...
def main():
  ...
  parser.add_argument(
    "--xml",
    type=str,
    default="../assets/g1/g1_sim2sim.xml",
    help="Path to MuJoCo XML file",
  )
  parser.add_argument(
    "--policy", type=str, required=True, help="Path to TWIST2 ONNX policy file"
  )
  parser.add_argument(
    "--device", type=str, default="cuda", help="Device to run policy on (cuda/cpu)"
  )
  parser.add_argument(
    "--record_video", action="store_true", help="Record video of simulation"
  )
  parser.add_argument(
    "--record_proprio", action="store_true", help="Record proprioceptive data"
  )
  ...
  parser.add_argument(
    "--policy_frequency", help="Policy frequency", default=100, type=int
  )
