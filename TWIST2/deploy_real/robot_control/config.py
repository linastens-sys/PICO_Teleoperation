...
class Config:
  def __init__(self, file_path) -> None:
    with open(file_path, "r") as f:
      ...
      self.ang_vel_scale = (
        config["ang_vel_scale"] if "ang_vel_scale" in config else None
      )
      self.dof_pos_scale = (
        config["dof_pos_scale"] if "dof_pos_scale" in config else None
      )
      self.dof_vel_scale = (
        config["dof_vel_scale"] if "dof_vel_scale" in config else None
      )
      self.action_scale = config["action_scale"] if "action_scale" in config else None
      self.cmd_scale = (
        np.array(config["cmd_scale"], dtype=np.float32)
