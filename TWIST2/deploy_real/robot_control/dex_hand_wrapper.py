...
class Dex3_1_Controller:
  def __init__(self, net, re_init=True):
    ...
    self.right_hand = ui.HandInterface.create_right_hand(
      net, False
    )  # Don't re-init for second hand
    ...
    self.get_hand_state()
    print(
      f"[Dex3_1_Controller] left_hand_state_array: {self.left_hand_state_array} \nright_hand_state_array: {self.right_hand_state_array}"
    )
    self.initialize()
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--net", type=str, default="eno1", help="Network interface used by G1RealWorldEnv."
  )
  ...
  for i in range(10):
    ...
    print(
      f"Step {i}: Left [{left_hand_state[0]:.3f}, {left_hand_state[1]:.3f}, {left_hand_state[2]:.3f}] Right [{right_hand_state[0]:.3f}, {right_hand_state[1]:.3f}, {right_hand_state[2]:.3f}]"
    )
    time.sleep(0.1)
