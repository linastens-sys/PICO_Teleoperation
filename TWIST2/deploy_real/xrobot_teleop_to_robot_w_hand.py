...
def start_interpolation(state_machine, start_obs, end_obs, duration=1.0):
  ...
  state_machine.interpolation_start_obs = (
    start_obs.copy() if start_obs is not None else None
  )
  state_machine.interpolation_target_obs = (
    end_obs.copy() if end_obs is not None else None
  )
...
def extract_mimic_obs_whole_body(qpos, last_qpos, dt=1 / 30):
  ...
  roll, pitch, yaw = euler_from_quaternion_np(
    root_quat.reshape(1, -1), scalar_first=True
  )
  # convert root vel to local frame
  base_vel_local = quat_rotate_inverse_np(root_quat, base_vel, scalar_first=True)
  base_ang_vel_local = quat_rotate_inverse_np(
    root_quat, base_ang_vel, scalar_first=True
  )
...
class StateMachine:
  ...
  def update(self, controller_data):
    ...
    left_index_trig_current = controller_data.get("LeftController", {}).get(
      "index_trig", False
    )
    ...
    left_axis_click_current = controller_data.get("LeftController", {}).get(
      "axis_click", False
    )
  ...
  def _update_velocity_commands(self, controller_data):
    ...
    if len(left_axis) >= 2 and len(right_axis) >= 2:
      ...
      self.velocity_commands[1] = (
        -left_axis[0] * xy_scale
      )  # left/right (x axis inverted)
      self.velocity_commands[2] = (
        -right_axis[0] * yaw_scale
      )  # yaw rotation (x axis inverted)
...
class XRobotTeleopToRobot:
  ...
  def _handle_enter_teleop(self, previous_state, current_retarget_obs):
    ...
    if previous_state == "idle":
      ...
      if (
        current_retarget_obs is not None
        and self.state_machine.last_mimic_obs is not None
      ):
        last_obs_35d = (
  ...
  def _get_teleop_mimic_obs(self, current_retarget_obs):
    ...
    if current_retarget_obs is not None:
      obs_35d = (
        current_retarget_obs[:35]
        if len(current_retarget_obs) > 35
  ...
  def send_to_redis(self, mimic_obs, neck_data=None):
    ...
    if self.redis_client is not None and mimic_obs is not None:
      ...
      self.redis_pipeline.set(
        "action_body_unitree_g1_with_hands", json.dumps(mimic_obs.tolist())
      )

    # Send hand action to redis
    if self.redis_client is not None:
      hand_left_pose, hand_right_pose = self.state_machine.get_hand_pose(
        self.robot_name
      )
      self.redis_pipeline.set(
        "action_hand_left_unitree_g1_with_hands", json.dumps(hand_left_pose.tolist())
      )
      self.redis_pipeline.set(
        "action_hand_right_unitree_g1_with_hands", json.dumps(hand_right_pose.tolist())
      )

    # Send neck data to redis
    if neck_data is not None:
      self.redis_pipeline.set(
        "action_neck_unitree_g1_with_hands", json.dumps(neck_data)
      )
  ...
  def send_controller_data_to_redis(self, controller_data):
    ...
    if self.redis_client is not None and controller_data is not None:
      self.redis_client.set(f"controller_data", json.dumps(controller_data))
