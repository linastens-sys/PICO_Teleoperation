...
# NOTE(mjlab): isaacgym import removed -- unused in this file, and isaacgym
# itself isn't installed in this environment (mjlab runs entirely on mujoco).
import torch
...
def build_mimic_obs(
  ...
  root_vel_local = quat_rotate_inverse_torch(
    root_rot, root_vel, scalar_first=False
  ).reshape(1, -1, 3)
  root_ang_vel_local = quat_rotate_inverse_torch(
    root_rot, root_ang_vel, scalar_first=False
  ).reshape(1, -1, 3)
  ...
  if mask_indicator:
    mimic_obs_buf = torch.cat(
      (
        ...
        roll,
        pitch,  # 2 dims (roll/pitch orientation)
...
def main(args, xml_file, robot_base):
  ...
  if args.use_remote_control:
    print(
      "[Motion Server] Remote control enabled. Waiting for start signal from robot controller..."
    )
  ...
  tar_motion_steps_tensor = torch.tensor(
    tar_motion_steps, device=device, dtype=torch.int
  )
  ...
  try:
    ...
    while True:
      ...
      if args.use_remote_control:
        ...
        if not motion_started and start_pressed:
          ...
          idle_mimic_obs = (
            start_frame_mimic_obs
            if args.send_start_frame_as_end_frame and start_frame_mimic_obs is not None
            else DEFAULT_MIMIC_OBS[args.robot]
          )
          redis_client.set(
            f"action_body_{args.robot}", json.dumps(idle_mimic_obs.tolist())
          )
          redis_client.set(
            f"action_hand_left_{args.robot}", json.dumps(np.zeros(7).tolist())
          )
          redis_client.set(
            f"action_hand_right_{args.robot}", json.dumps(np.zeros(7).tolist())
          )
      ...
      mimic_obs_list = (
        mimic_obs.tolist() if mimic_obs.ndim == 1 else mimic_obs.flatten().tolist()
      )
      ...
      # Print or log it
      print(
        f"Step {t_step:4d} => mimic_obs shape = {mimic_obs.shape} published...",
        end="\r",
      )
    ...
    for i in range(int(time_back_to_default / control_dt)):
      interp_mimic_obs = last_mimic_obs + (target_mimic_obs - last_mimic_obs) * (
        i / (time_back_to_default / control_dt)
      )
      redis_client.set(
        f"action_body_{args.robot}", json.dumps(interp_mimic_obs.tolist())
      )
