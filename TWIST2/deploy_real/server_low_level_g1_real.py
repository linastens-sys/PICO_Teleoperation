    ...
    try:
      while True:
        ...
        if self.use_hand:
          ...
          lh_pos, rh_pos, lh_temp, rh_temp, lh_tau, rh_tau = (
            self.hand_ctrl.get_hand_all_state()
          )
          hand_left_json = json.dumps(left_hand_state.tolist())
          hand_right_json = json.dumps(right_hand_state.tolist())
          self.redis_pipeline.set(
            "state_hand_left_unitree_g1_with_hands", hand_left_json
          )
          self.redis_pipeline.set(
            "state_hand_right_unitree_g1_with_hands", hand_right_json
          )
        ...
        if self.body_smoother is not None:
          action_mimic = self.body_smoother.smooth(
            np.array(action_mimic, dtype=np.float32)
          )
...
def main():
  ...
  parser.add_argument(
    "--config",
    type=str,
    default="robot_control/configs/g1.yaml",
  ...
  parser.add_argument("--use_hand", action="store_true", help="Enable hand control")
  parser.add_argument(
