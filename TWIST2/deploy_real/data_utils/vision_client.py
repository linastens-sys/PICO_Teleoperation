...
class VisionClient:
  def __init__(
    ...
    if (self.img_shape is not None) and (self.img_shm_name is not None):
      ...
      self.img_array = np.ndarray(
        self.img_shape, dtype=np.uint8, buffer=self.img_shm.buf
      )
    ...
    if (self.depth_shape is not None) and (self.depth_shm_name is not None):
      ...
      self.depth_array = np.ndarray(
        self.depth_shape, dtype=np.float32, buffer=self.depth_shm.buf
      )
  ...
  def _update_performance_metrics(self, print_info, verbose=False):
    ...
    if self.frame_count % 30 == 0:
      ...
      if verbose:
        print(
          f"[VisionClient] FPS: {real_time_fps:.2f}, Frames: {self.frame_count}, Elapsed: {elapsed:.2f}s"
        )
        print(print_info, "\n")
  ...
  def handle_depth_image(self, depth_img):
    ...
    if self.depth_show:
      ...
      depth_colormap = cv2.applyColorMap(
        cv2.convertScaleAbs(temp, alpha=255.0 / max_depth), cv2.COLORMAP_JET
      )
  ...
  def receive_process(self):
    ...
    print(
      f"[VisionClient] Subscribed to tcp://{self.server_address}:{self.port}. Waiting for data..."
    )
      ...
      while self.running:
        try:
          ...
          if actual_jpeg_size != jpeg_length:
            if verbose:
              print(
                f"[Warning] JPEG size mismatch: expected {jpeg_length}, got {actual_jpeg_size}"
              )
            continue
          ...
          if verbose:
            print(
              f"==> JPEG decoded image.shape: {image.shape}, original JPEG size: {jpeg_length} bytes"
            )
...
if __name__ == "__main__":
  ...
  depth_array = np.ndarray(
    depth_shape, dtype=np.float32, buffer=depth_shared_memory.buf
  )
